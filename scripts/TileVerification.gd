...
static func verify_all_tiles() -> Dictionary:
	...
	var results = {
		"total": BoardTileHelperScript.TOTAL_TILES,
		"found": 0,
		"missing": 0,
		"missing_tiles": []
	}
	...
	for i in range(1, BoardTileHelperScript.TOTAL_TILES + 1):
...
static func verify_specific_positions():
	...
	print("\nGrid Enemigo (0,0):")
	var enemy_tile = BoardTileHelperScript.get_enemy_tile_index(0, 0)
	print("  Índice calculado: %d" % enemy_tile)
	verify_tile_exists(enemy_tile)
	
	# Grid Aliado (0,0) -> tile 56
	print("\nGrid Aliado (0,0):")
	var ally_tile = BoardTileHelperScript.get_ally_tile_index(0, 0)
