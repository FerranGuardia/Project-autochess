...
func create_grid():
	...
	if not background:
		...
		if arena_texture != null:
			...
			# Primera celda está en: x = (0 - 3.5 + 0.5) * 100 - 50 = -300
			# Última celda termina en: x = (6 - 3.5 + 0.5) * 100 + 50 = 350
			# El sprite debe empezar en -350 (esquina superior izquierda)
			...
			background = sprite
			print("✓ Arena aliada cargada desde sprite (posicionada correctamente)")
		else:
